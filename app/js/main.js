$(function(){
	...
	var git = new Github({
		...
		"token": "d7a76a686d0b36af0ce806e5ca56daa7dd20fb1d"
	});
	...
	git.getUserLangages("emoc11", function(res) {
		res_lang = res["data"];
		res_total = new Intl.NumberFormat().format(res["total"]);

		$(".line_total").html("<span>Total :</span> "+res_total+" lines");

		// PIE CHART
		var w = h = $(window).innerHeight() / 2;
		var r = w/2;
		var maxV = d3.max(res_lang, function(d) { return +d.val;} );
		var minV = d3.min(res_lang, function(d) { return +d.val;} );
		...
		var arc = d3.arc()
       	...
       	var arcs = vis.selectAll("g.slice")
	        ...
	        arcs.append("svg:text")
		            ...
		            // .text(function(d, i) {return res_lang[i].lang+" : "+res_lang[i].val+" lines"; });
		            .text(function(d, i) {return new Intl.NumberFormat().format(res_lang[i].val)+" lines"; });
